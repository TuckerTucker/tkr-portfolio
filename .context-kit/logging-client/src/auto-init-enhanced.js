/**
 * Enhanced Auto-Initialization for tkr-context-kit logging
...
class EnhancedTkrLogger {
  ...
  log(level, message, metadata = {}, component = '') {
    ...
    // Prepare log data for batching
    const logData = {
      timestamp: Math.floor(Date.now() / 1000),
      level: level.toUpperCase(),
      service: this.baseLogger.config.service,
      service_type: this.baseLogger.config.serviceType,
      message,
      metadata: enrichedMetadata,
      component: component || this.baseLogger.config.defaultComponent,
      baseUrl: this.baseLogger.config.baseUrl
    };
    ...
    if (this.baseLogger.config.logToConsole) {
      ...
      if (console[consoleMethod]) {
        console[consoleMethod](
          `[${level.toUpperCase()}] ${logData.service}/${logData.component}: ${message}`,
          metadata
        );
      }
    }
