...
export class KGDatabaseSimple {
  ...
  getStats() {
    ...
    // Add logging stats
    const logEntryCount = this.db.prepare('SELECT COUNT(*) as count FROM log_entries').get() as { count: number };
    const logSourceCount = this.db.prepare('SELECT COUNT(*) as count FROM log_sources').get() as { count: number };
